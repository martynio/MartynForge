document.addEventListener('DOMContentLoaded', function () {
 ...
 const input = document.getElementById("aiInput");
  ...
  const fakeAI = (query) => {
    ...
    if (q.includes("logo")) return "Tworzę wyjątkowe logotypy, które pasują do Twojej wizji.";
    if (q.includes("strona") || q.includes("website")) return "Projektuję nowoczesne, responsywne strony internetowe.";
    if (q.includes("cena") || q.includes("koszt")) return "Ceny są elastyczne – napisz, co potrzebujesz, a znajdziemy rozwiązanie.";
    return "Zapytanie przyjęte. Jestem tu, by pomóc. Możesz pytać o ofertę, usługi, projekty i więcej.";
  };
  ...
  const handleSearch = () => {
    ...
    if (isQuestion) {
      ...
      // domyślna akcja – np. scroll do sekcji usług
